...
class frmLogin extends DbConn
{
    public function checkLogin($myusername, $mypassword)
    {
        ...
        $stmt = $db->conn->prepare("SELECT U.*, R.Priority FROM ".$tbl_members." U LEFT JOIN refroles R ON R.RoleId = U.RoleId WHERE U.username = :myusername");
        ...
        if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
            ...
            // Stopped with the Checks - SM 23 Jan 2018, 11:39
            ...
            if (password_verify($mypassword, $result['password']) && $result['verified'] == '1') {
                ...
                $success = 'true';
                    ...
                    $_SESSION['fullName'] = trim($result['FirstName'] ." ". $result['LastName']);
                    $_SESSION['JobTitle'] = trim($result['JobTitle']);
                    $_SESSION['DateEngaged'] = $result['DateEngaged'];
                    $_SESSION['RolePriority'] = $result['Priority'];
    ...
    public function insertAttempt($username)
    {
        try {
            ...
            $stmt = $db->conn->prepare("INSERT INTO ".$tbl_attempts." (ipaddress, attempts, lastlogin, username) values(:ip, 1, :lastlogin, :username)");
        ...
        try {
            ...
            if ($curr_attempts >= $max_attempts && $timeDiff < $login_timeout) {
                ...
                if ($timeDiff >= $login_timeout) {
                    ...
                    $sql = "UPDATE ".$tbl_attempts." SET attempts = :attempts, lastlogin = :lastlogin where ipaddress = :ip and username = :username";
                    $curr_attempts = 1;
